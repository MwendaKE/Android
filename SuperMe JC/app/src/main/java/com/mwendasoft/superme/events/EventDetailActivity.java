...
import com.mwendasoft.superme.core.SuperMeTimeHelper;
...
public class EventDetailActivity extends BaseActivity {
    ...
    private TextView eventDetailTitle, eventDateAndTimeView, 
	eventAddressView, eventBudgetAndAttendedView, eventDetailNotesView;
    private ImageButton editEventFab;
    private LinearLayout eventImageViewLayout, eventVideoViewLayout, eventVoicesViewLayout;
    private LinearLayout eventImageContainer, eventVideoContainer, eventVoicesContainer;
    ...
    private void initializeViews() {
        ...
        eventDateAndTimeView = findViewById(R.id.eventDateAndTimeView);
        eventAddressView = findViewById(R.id.eventAddressView);
        eventBudgetAndAttendedView = findViewById(R.id.eventBudgetAndAttendedView);
        eventDetailNotesView = findViewById(R.id.eventDetailNotesView);
        editEventFab = findViewById(R.id.editEventFab);

        eventImageViewLayout = findViewById(R.id.eventImageViewLayout);
        eventVideoViewLayout = findViewById(R.id.eventVideoViewLayout);
        eventVoicesViewLayout = findViewById(R.id.eventVoicesViewLayout);

        eventImageContainer = findViewById(R.id.eventImageContainer);
        eventVideoContainer = findViewById(R.id.eventVideoContainer);
        eventVoicesContainer = findViewById(R.id.eventVoicesContainer);
    }
    ...
    private void loadEvent() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            float budget = cursor.getFloat(cursor.getColumnIndex("budget"));
            String notes = cursor.getString(cursor.getColumnIndex("notes"));
            int attended = cursor.getInt(cursor.getColumnIndex("attended"));

            updateEventUI(title, date, time, address, budget, notes, attended);
        }
        if (cursor != null) {
            cursor.close();
        }
    ...
    private void updateEventUI(String title, String date, String time, String address, 
							   float budget, String notes, int attended) {
        appTimeHelper = new SuperMeTimeHelper(date, time);
        String formattedDateTime = appTimeHelper.getFormattedDate() + ", " + appTimeHelper.getFormattedTime();
        String relativeTime = appTimeHelper.getRelativeTime();
        String attendanceStatus = attended == 1 ? "Attended" : "Not Attended";

        eventDetailTitle.setText(title);
        eventDateAndTimeView.setText(formattedDateTime + " | " + relativeTime);
        eventAddressView.setText(address);
        eventBudgetAndAttendedView.setText(String.format("Budget: $%.2f | Status: %s", budget, attendanceStatus));
        eventDetailNotesView.setText(notes);
    }
    ...
    private void loadImages() {
        imagePaths = dbHelper.getMediaPaths(eventId, "image");

        if (imagePaths == null || imagePaths.isEmpty()) {
            hideMediaLayout(eventImageViewLayout);
            return;
        }
        ...
        runOnUiThread(new Runnable() {
				@Override
				public void run() {
					showMediaLayout(eventImageViewLayout);
    ...
    private void loadVideos() {
        final List<String> videoPaths = dbHelper.getMediaPaths(eventId, "video");

        if (videoPaths == null || videoPaths.isEmpty()) {
            hideMediaLayout(eventVideoViewLayout);
            return;
        }
        ...
        runOnUiThread(new Runnable() {
				@Override
				public void run() {
					showMediaLayout(eventVideoViewLayout);
					eventVideoContainer.removeAllViews();
    ...
    private void loadAudios() {
        final Set<String> audioPaths = new LinkedHashSet<>(dbHelper.getMediaPaths(eventId, "audio"));

        if (audioPaths == null || audioPaths.isEmpty()) {
            hideMediaLayout(eventVoicesViewLayout);
            return;
        }
        ...
        runOnUiThread(new Runnable() {
				@Override
				public void run() {
					showMediaLayout(eventVoicesViewLayout);
					eventVoicesContainer.removeAllViews();
        ...
        imageView.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					DialogMultipleImageViewHelper dialog = 
						new DialogMultipleImageViewHelper(EventDetailActivity.this, imagePaths);
					dialog.showImageDialog(path);
				}
			});

        eventImageContainer.addView(imageView);

        if (position < total - 1) {
            eventImageContainer.addView(createSeparator());
        }
    }
    ...
    private void addVideoToLayout(final String path, Bitmap thumbnail, int position, int total) {
        ...
        videoContainer.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					videoViewer.showVideoDialog(EventDetailActivity.this, path);
				}
			});

        eventVideoContainer.addView(videoContainer);

        if (position < total - 1) {
            eventVideoContainer.addView(createSeparator());
        }
    }
    ...
    private void addAudioToLayout(final String path, int position, int total) {
        ...
        View audioView = LayoutInflater.from(this).inflate(R.layout.audio_player_helper, eventVoicesContainer, false);
        ...
        btnPlayPause.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					try {
						...
						e.printStackTrace();
						Toast.makeText(EventDetailActivity.this, "Error playing audio", Toast.LENGTH_SHORT).show();
        ...
        eventVoicesContainer.addView(audioView);

        if (position < total - 1) {
            eventVoicesContainer.addView(createSeparator());
        }
    }
